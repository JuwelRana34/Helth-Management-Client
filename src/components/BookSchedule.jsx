  ...
  const fetchSchedule = async () => {
    ...
    try {
      const res = await axiosSecure.get(
        ...
        }/api/schedule?doctorId=${doctorId}&date=${selectedDate}`
      );
  ...
  return (
   <div className=" lg:flex gap-5">
    <div className=" md:w-1/2 mx-auto">
       <img className=" w-full" src="https://i.ibb.co.com/DHXff26R/doctor-doing-teeth-treatment-to-patient.png" alt="Photo"  />
    </div>
        ...
        <div className="space-y-4">
          {slots?.length > 0 ? (
            slots?.map((slot) => (
              <div
                key={slot.time}
                className="flex justify-between items-center bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm hover:bg-emerald-50 transition duration-200 space-x-2"
              >
      ...
      {!isLoading ? (
        <div className="mt-6 space-y-4">
          {BookedSlots?.map((item, index) => (
            <div
              key={index}
              className="border p-4 rounded-lg bg-emerald-50 shadow-md"
            >
