...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        if (isAdmin) {
            ...
            socket.on("newMessageFromUser", ({ userEmail, messages: newMessages }) => {
                ...
                setActiveUsers((prevUsers) => {
                    ...
                    if (userExists) {
                        return prevUsers.map((u) => (u.email === userEmail ? { ...u, hasNewMessage: true } : u));
                    }
                    return prevUsers;
                });
            });

            socket.on("loadConversation", ({ userEmail, messages: loadedMessages }) => {
                if (selectedUser?.email === userEmail) {
    ...
    const sendMessage = () => {
        if (message.trim() !== "" && user) {
            if (isAdmin && selectedUser) {
                socket.emit("sendMessageToUser", { receiver: selectedUser.email, message });
    ...
    const handleUserSelection = (user) => {
        ...
        setActiveUsers((prevUsers) => prevUsers.map((u) => ({ ...u, hasNewMessage: false })));
    };
                        ...
                        {activeUsers.map((u) => (
                            <li
                                ...
                                className={`flex items-center gap-2 cursor-pointer p-2 rounded-lg text-sm font-medium ${
                                    selectedUser?.email === u.email ? "bg-blue-500 text-white" : "bg-gray-300"
                                }`}
                                onClick={() => handleUserSelection(u)}
                            >
                                <img src={u.photo} alt="User" className="w-8 h-8 rounded-full" />
                                <span>{u.name}</span>
                                {u.hasNewMessage && <span className="ml-2 text-xs text-red-500 font-bold">New!</span>}
                            </li>
                        ))}
                    </ul>
                </div>
                ...
                <div className="flex-grow overflow-y-auto bg-gray-50 p-3 rounded-lg">
                    {messages.map((msg, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={`flex flex-col ${isYou ? "items-end" : "items-start"} mb-2`}
                            >
                                <div
